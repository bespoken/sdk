...
const metrics = Metrics.instance(inputFile.split('.')[0])
metrics.initialize(() => {
...
utterances.forEach(async (record) => {
  ...
  if (Config.has('sequence')) {
    ...
    for (const command of commands) {
      ...
      console.log(`SEQUENCE STEP ${step} TRANSCRIPT: ${sequenceResponse.transcript}`)
      step++
    }
  }

  const response = await device.message(token, `${utterance}`)
  console.log('TEST RESPONSE: ' + response.transcript)
